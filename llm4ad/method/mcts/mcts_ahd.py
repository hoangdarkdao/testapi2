...
from .profiler import MCTSProfiler
...
class MCTS_AHD:
    ...
    def _sample_evaluate_register(self, prompt, func_only=False):
        ...
        if self._profiler is not None:
            self._profiler.register_function(func, program=str(program)) # write to sample_best.json
            if isinstance(self._profiler, MCTSProfiler):
                self._profiler.register_population(self._population) # write to pop_.json
    ...
    def check_duplicate_obj(self, population: list[MCTSNode], code):
        for ind in population:
    ...
    def population_management_s1(self, pop_input, size):
        '''
            individual is from code.py, str(individual) get the function like select next nodes
        '''
        unique_pop = []
        unique_algorithms = []
    ...
    def expand(self, mcts: MCTS, node_set: list[MCTSNode], cur_node: MCTSNode, option: str):
        '''
            Args:
                node_set: like population, return list of MCTSNode in a pathx
        '''
        is_valid_func = True
        if option == 's1':
            path_set = []
    ...
    def _iteratively_init_population_root(self):
        ...
        """
        print("Inside iterative init population")
    ...
    def run(self):
        ...
        for indiv in self._population.population:
            print(f"Update mcts, individual is {indiv}")
        ...
        while self._continue_loop():
            node_set = [] # the population, list of MCTSNode we need to update over population
            ...
            while len(cur_node.children) > 0 and cur_node.depth < mcts.max_depth:
                ...
                if int((cur_node.visits) ** mcts.alpha) > len(cur_node.children): # progressive widening
